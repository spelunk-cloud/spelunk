...
1. Builds `spelunk`, `spelunk-server` and `spelunk-export` release binaries for all supported platforms.
2. Strips binaries where possible to reduce download size.
3. Packages each platform's `spelunk` + `spelunk-server` into a `.tar.gz` archive.
4. Packages each platform's `spelunk-export` into a second, separate archive.
5. Builds an `amd64` Debian package (`spelunk_<version>_amd64.deb`).
6. Creates a GitHub Release and attaches every `.tar.gz` / `.zip` archive and the `.deb` as downloadable assets.
7. Auto-generates release notes from merged pull requests and commits.

`spelunk-export` is published as its own per-platform asset and is
deliberately not in the product archive, the `.deb`, the Homebrew formula or
the Scoop manifest. It is a read-only tool meant to be fetched on its own,
verified, run against a store and deleted again; putting it on `PATH` or
requiring a download of the whole product to get it would defeat that. It
depends on no other crate in the workspace and links no system SQLite
(`rusqlite`'s `bundled` feature), so nothing needs installing alongside it.

Asset checksums are not generated by this workflow. GitHub records a `digest`
field shaped `sha256:<hex>` on every release asset at upload time, readable per
asset without downloading it; the `update-homebrew-formula` and
`update-scoop-manifest` jobs consume exactly that.
...
It builds `spelunk` + `spelunk-server` + `spelunk-export` inside `debian:11`
(the glibc 2.31 floor), runs the same glibc-ceiling check as CI, assembles and
builds the `.deb` (with `Depends` derived inside `debian:11`, matching the
workflow), packages the standalone `spelunk-export` archive and checks with
`ldd` that it links neither a system SQLite nor `libdbus`, then installs +
smoke-tests the result in fresh `debian:11` / `ubuntu:20.04` / `ubuntu:24.04`
containers.
...
on the support floor, and the standalone `spelunk-export` asset reads a
real store on those same images.
...
# Export tool, published separately per platform
https://github.com/spelunk-cloud/spelunk/releases/download/<version>/spelunk-export-<version>-<target>.tar.gz
https://github.com/spelunk-cloud/spelunk/releases/download/<version>/spelunk-export-<version>-x86_64-pc-windows-msvc.zip
```

Read an asset's sha256 without downloading it, to verify a download against
what the release actually published:

```bash
gh release view <version> --repo spelunk-cloud/spelunk --json assets \
