//! Shared test-only helpers for `entity_id_migration`'s `migration_tests` and
//! `collision_recovery_tests` submodules. Moved out of an inline `mod tests`
//! block during the file-size refactor that split `entity_id_migration.rs`
//! into this directory; no behavior changed.
