...
impl ServerDb {
    ...
    /// The generated bytes are formatted as a proper UUID v4 string with
    /// the version (4) and variant bits set.
...
/// Format a 32-char lowercase hex string as a UUID v4 string.
///
/// Sets version nibble to `4` (byte 6 upper nibble) and variant bits to `10`
/// (byte 8 upper two bits), then inserts dashes in the standard positions.
fn format_uuid_v4(hex: &str) -> String {
    // hex must be exactly 32 lowercase hex chars from randomblob(16).
    // Work on bytes for efficiency; all chars are ASCII.
    let b = hex.as_bytes();
    // Set version = 4 at nibble index 12 (byte 6, upper nibble).
    // Set variant = 10xx at nibble index 16 (byte 8, upper nibble).
    let version_nibble = b'4';
